...
class LipschitzLinear(torch.nn.Module):
    def __init__(self, in_features, out_features, zero=False):
        ...
        self.weight = torch.nn.Parameter(torch.zeros((out_features, in_features), requires_grad=True))
        self.bias = torch.nn.Parameter(torch.zeros((out_features), requires_grad=True))
        self.c = torch.nn.Parameter(torch.zeros((1), requires_grad=True))
        self.softplus = torch.nn.Softplus()

        self.zero = zero
        with torch.no_grad():
            self.weight.add_(1e-6)
...
class MLPLip(nn.Module):
    ...
    def forward(self, x, rotations, scales, means, opacity, c=1.0, target="xc"): 
        ...
        C = torch.ones_like(opacity, requires_grad=False).fill_(c)
        C_emb = embed_C_fn(C)
