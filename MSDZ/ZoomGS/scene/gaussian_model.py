...
from utils.compat import resolve_device
...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        self._xyz = nn.Parameter(self._xyz.to(self.device).requires_grad_(True))
        self._features_dc = nn.Parameter(self._features_dc.to(self.device).requires_grad_(True))
        self._features_rest = nn.Parameter(self._features_rest.to(self.device).requires_grad_(True))
        self._scaling = nn.Parameter(self._scaling.to(self.device).requires_grad_(True))
        self._rotation = nn.Parameter(self._rotation.to(self.device).requires_grad_(True))
        self._opacity = nn.Parameter(self._opacity.to(self.device).requires_grad_(True))
        ...
        if mlp_state_dict is not None:
            self._mlp.load_state_dict(mlp_state_dict)
        if mlp_opt_dict is not None:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_point_cloud = torch.as_tensor(np.asarray(pcd.points), dtype=torch.float32, device=self.device)
        fused_color = RGB2SH(torch.as_tensor(np.asarray(pcd.colors), dtype=torch.float32, device=self.device))
        ...
        self._mlp = MLP(58 + 1, 3).to(self.device)
