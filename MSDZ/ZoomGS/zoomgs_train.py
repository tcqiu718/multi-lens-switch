...
from utils.compat import load_checkpoint, maybe_empty_cache, set_runtime_device
import uuid
...
def training(dataset, opt, pipe, args):
    ...
    if args.stage == "uw_pretrain":
        ...
        if checkpoint:
            (model_params, first_iter) = load_checkpoint(checkpoint, map_location=runtime_device)
        ...
        for iteration in range(first_iter, opt.iterations + 1):
            ...
            gt_image_wide = viewpoint_cam_wide.original_image.to(runtime_device)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        maybe_empty_cache(scene.gaussians.device)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    gt_image = torch.clamp(viewpoint.original_image.to(scene.gaussians.device), 0.0, 1.0)
