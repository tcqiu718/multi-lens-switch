...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
    ...
    Background tensor (bg_color) must be on the same device as the Gaussian model.
    """
    ...
    confidence = torch.ones((pc.get_xyz.shape[0], 1), dtype=pc.get_xyz.dtype, device=pc.get_xyz.device)
